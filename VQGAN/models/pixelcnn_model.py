...
import random
import torch
from basicsr.data.degradations import random_add_gaussian_noise_pt, random_add_poisson_noise_pt, random_add_gaussian_noise, random_add_poisson_noise
...
class PixelCNN_Model(SRModel):

    def __init__(self, opt):
        super(PixelCNN_Model, self).__init__(opt)
        self.net_vae = build_network(opt['network_vae'])
        self.net_vae = self.model_to_device(self.net_vae)
        self.scale = self.opt.get('scale', 1)
        
        # load pretrained models
        load_path = self.opt['path'].get('pretrain_network_vae', None)
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_g', 'params')
            self.load_network(self.net_vae, load_path, True, param_key)
        ...
        self.cri_xe = nn.CrossEntropyLoss()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(PixelCNN_Model, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
    ...
    def optimize_parameters(self, current_iter):
        ...
        with torch.no_grad():
            ze = self.net_vae.module.encode_idx(self.gt)
        self.output = self.net_g(ze)
        l_xe = self.cri_xe(self.output, ze)
        l_total += l_xe
        loss_dict['l_xe'] = l_xe
